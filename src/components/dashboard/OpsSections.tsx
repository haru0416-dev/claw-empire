...
import { getRankTier, timeAgo, type TFunction } from "./model";
        ...
        <div className="space-y-2">
          {entries.map((entry) => {
            return (
              <article
                key={entry.id}
                className={`group grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl border border-white/[0.06] border-l-[3px] ${entry.leftBorderClassName} bg-white/[0.02] p-3 transition-all duration-200 hover:bg-white/[0.04] hover:translate-x-1`}
              >
                {entry.agent ? (
                  <AgentAvatar agent={entry.agent} agents={agents} size={36} rounded="xl" />
                ) : (
                  <div
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 flex items-center gap-1.5 text-[10px]" style={{ color: "var(--th-text-muted)" }}>
                    <span className={`h-1.5 w-1.5 flex-shrink-0 rounded-full ${entry.dotClassName}`} />
                    {entry.actorLabel}
                  </p>
                  <p className="mt-1 truncate text-[11px]" style={{ color: "var(--th-text-secondary)" }}>
                    {entry.body}
                  </p>
                </div>

                <div className="flex flex-col items-end gap-1">
                  <span className={`rounded-md border px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${entry.badgeClassName}`}>
                    {entry.badgeLabel}
                  </span>
                  <span className="text-[9px] font-medium" style={{ color: "var(--th-text-muted)" }}>
                    {timeAgo(entry.timestamp, localeTag)}
                  </span>
                </div>
              </article>
            );
